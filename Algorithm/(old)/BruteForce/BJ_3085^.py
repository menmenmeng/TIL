...
<해설>
2차원 array를 사용할 때, row와 column 관점에서 하나의 행, 하나의 열을 떼어낼 생각을 하지 마라.
그냥 2차원 보드 상에서 해결할 수 있다.
후보1의 check_h와 check_v 함수가 모양이 다른 이유는 내가 row, column관점에서 행과 열을 떼어낼 생각을 했기 때문이다.
정답지의 check함수는 통일성이 있게 생겼으며 알아보기도 쉽다.
'''
# 후보 1
# 존나게 길고 존나 비효율적이다.
'''
